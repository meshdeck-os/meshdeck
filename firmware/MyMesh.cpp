...
void MyMesh::logRxRaw(float snr, float rssi, const uint8_t raw[], int len) {
  if (_ui) _ui->onRawRx(snr, rssi, len);   // MeshDeck hook
...
void MyMesh::onDiscoveredContact(ContactInfo &contact, bool is_new, uint8_t path_len, const uint8_t* path) {
  ...
  if (_ui) _ui->onAdvertSeen(contact, is_new, path_len);   // MeshDeck hook
  ...
  for (int i = 0; i < EXPECTED_ACK_TABLE_SIZE; i++) {
    if (memcmp(data, &expected_ack_table[i].ack, 4) == 0) { // got an ACK from recipient
      ...
      if (_ui) {   // MeshDeck hook
        uint32_t a;
        memcpy(&a, data, 4);
        _ui->onAckDelivered(a, expected_ack_table[i].contact, trip_time);
      }
...
void MyMesh::onChannelMessageRecv(const mesh::GroupChannel &channel, mesh::Packet *pkt, uint32_t timestamp,
  ...
  if (_ui) {   // MeshDeck hook
    ...
    if (getChannel(channel_idx, ch_details)) {
      ch_name = ch_details.name;
    }
    _ui->onChannelMsg(channel_idx, ch_name, text, timestamp, path_len, pkt->getSNR());
  }
}
...
void MyMesh::onTraceRecv(mesh::Packet *packet, uint32_t tag, uint32_t auth_code, uint8_t flags,
  ...
  if (_ui) {   // MeshDeck hook
    _ui->onTraceResult(tag, path_len, path_hashes, path_snrs, path_len >> path_sz, packet->getSNR());
  }
...
void MyMesh::onSendTimeout() {
  if (_ui) _ui->onSendTimeout();   // MeshDeck hook
}

// ---- MeshDeck additions ----

void MyMesh::registerExpectedAck(uint32_t expected_ack, ContactInfo* contact) {
  if (expected_ack) {
...
uint32_t MyMesh::sendTracePath(const uint8_t* path, uint8_t path_len) {
  uint32_t tag = getRNG()->nextInt(1, 0x7FFFFFFF);
  auto pkt = createTrace(tag, 0, 0);   // flags=0 -> one SNR byte per hop
  if (pkt) {
...
int MyMesh::getNoiseFloorNow() const {
  return _radio->getNoiseFloor();
}
...
float MyMesh::getLastRSSI() const { return _radio->getLastRSSI(); }
float MyMesh::getLastSNR() const { return _radio->getLastSNR(); }
